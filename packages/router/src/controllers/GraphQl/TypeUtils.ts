          ...
          if ((fields[field] as any).type) {
            ...
            if (typeof (fields[field] as any).type === 'string') {
              if ((fields[field] as any).type === 'Relation') {
                ...
                typeString +=
                  field +
                  ': ' +
                  getGraphQLType((fields[field] as any).type) +
                  ((fields[field] as any).required ? '!' : '') +
                  ' ';
              }
            }
            // if type is an array
              ...
              typeString +=
                ...
                ' ';
              finalString +=
                ' ' + extractTypesInternal(nestedName, (fields[field] as any).type) + ' ';
            }
          } else {
            // object of some kind
            let nestedName = constructName(name, field);
            ...
            typeString += field + ': ' + nestedName + ' ';
            finalString +=
              ' ' + extractTypesInternal(nestedName, fields[field] as any) + ' ';
          }
        }
      }
    }
