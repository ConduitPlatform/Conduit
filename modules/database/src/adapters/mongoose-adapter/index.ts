...
export class MongooseAdapter extends DatabaseAdapter<MongooseSchema> {
  ...
  async registerDeclaredSchema(schema: ConduitSchema) {
    if (schema.collectionName.startsWith('cnd_')) {
      const nameWithoutPrefix = schema.collectionName.replace('cnd_', '_');
      const collections = await this.mongoose.connection.db.listCollections().toArray();
      const declaredSchema = collections.find(c => c.name === nameWithoutPrefix);
      if (declaredSchema) {
        (schema.collectionName as any) = nameWithoutPrefix;
      }
    }
    return await this.createSchemaFromAdapter(schema);
  }
