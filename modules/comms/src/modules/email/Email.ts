...
export default class Email implements CommService {
  readonly functions = {
  ...
  static getInstance(grpcSdk?: ConduitGrpcSdk) {
    if (!Email._instance) {
      ...
      Email._instance = new Email(grpcSdk);
    }
  ...
  async preConfig(config: Config) {
    if (config.email.transportSettings?.sendgrid.hasOwnProperty('apiUser')) {
      delete (
  ...
  async onConfig() {
    if (!ConfigController.getInstance().config.email.active) {
      ...
      if (!this.isRunning) {
        ...
        this.adminRouter = new AdminHandlers(this.grpcSdk);
      ...
      this._health = HealthCheckStatus.SERVING;

      const config = ConfigController.getInstance().config as Config;
      if (
        config.email.storeEmails.storage.enabled &&
        !this.grpcSdk.isAvailable('storage')
      ) {
  ...
  async sendEmail(
    ...
    const params: ISendEmailParams = {
      ...
      sendingDomain: emailConfig?.email.sendingDomain ?? '',
    };
  ...
  private async handleEmailCleanupJob(config: Config) {
    ...
    if (
      !config.email.storeEmails.enabled ||
      !config.email.storeEmails.cleanupSettings.enabled
    ) {
    ...
    await this.emailCleanupQueue.add(
      'cleanup',
      {
        limit: config.email.storeEmails.cleanupSettings.limit,
        deleteStorageFiles: config.email.storeEmails.storage.enabled,
      },
      {
        repeat: {
          every: config.email.storeEmails.cleanupSettings.repeat,
        },
      },
    );
  }
