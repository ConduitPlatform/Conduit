...
export class PaymentsRoutes {
    ...
    constructor(server: GrpcServer, private readonly grpcSdk: ConduitGrpcSdk) {
        ...
        server.addService(path.resolve(__dirname, "./router.proto"), "payments.router.Router", {
    ...
    async getSubscriptions(call: any, callback: any) {
        ...
        const subscriptions = await this.database.findMany('Subscription', {
            userId: context.user._id,
            activeUntil: {$gte: new Date()}
        }, null, null, null, null, 'product')
            .catch((e: Error) => {
                errorMessage = e.message;
            });
        if (!isNil(errorMessage)) {
