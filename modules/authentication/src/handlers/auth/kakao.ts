...
export class KakaoHandlers {
    ...
    async authenticate(call: any, callback: any) {
        ...
        try {
            const response = await axios.post('https://kauth.kakao.com/oauth/token', querystring.stringify({
        ...
        let user = await this.database.findOne('User', {'kakao.id': userInfo.id}).catch((e: any) => errorMessage = e.message);
        if (!isNil(errorMessage)) return callback({code: grpc.status.INTERNAL, message: errorMessage});
