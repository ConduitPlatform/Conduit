...
export class CommonHandlers {
  ...
  async logOut(call: ParsedRouterRequest): Promise<UnparsedRouterResponse> {
    ...
    const namedConfig = config.clients.named;
    const unnamedConfig = config.clients.unnamed;

    await AuthUtils.logOutClientOperations(this.grpcSdk, namedConfig, unnamedConfig, authToken, clientId, user._id);
